...
module ctrl(
    ...
    input  wire                  hold_flag_mem,  // MEM 级暂停请求（新增）
    ...
    always @(*) begin
        ...
        // 优先级：JTAG 停止 > EX/MEM 跳转/暂停 > CLINT 中断 > 总线暂停
        if (jtag_halt_flag == `HOLD_ENA) begin
            // JTAG调试请求具有最高优先级
            hold_flag_o = `HOLD_ID;
        end
        else if (jump_flag_i == `JUMP_ENA || hold_flag_ex == `HOLD_ENA || hold_flag_mem == `HOLD_ENA) begin
            // EX或MEM级请求跳转或暂停都要停整个流水线
            // 添加了MEM级的暂停请求
            hold_flag_o = `HOLD_ID;
        end
